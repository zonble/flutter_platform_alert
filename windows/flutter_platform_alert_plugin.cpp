...
namespace {
	...
	std::string GetStringArgument(const flutter::MethodCall<>& method_call, const char* name) {
		std::string url;
	...
	void FlutterPlatformAlertPlugin::RegisterWithRegistrar(
		...
		auto channel =
			std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
				registrar->messenger(), "flutter_platform_alert",
	...
	void FlutterPlatformAlertPlugin::HandleMethodCall(
		...
		if (method_call.method_name().compare("playAlertSound") == 0) {
			...
			std::string iconStyleString = GetStringArgument(method_call, "iconStyleString");
